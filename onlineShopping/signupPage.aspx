<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="signupPage.aspx.cs" Inherits="onlineShopping.signupPage" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>

    </title>
    <style>
        .signup{
            border:solid;
            border-color:black;
            font-size:30px;
            background-color:aliceblue;
            position: relative;
            left: 600px;
            top: -60px;
            width: 700px;
            height: 750px;
            text-align:start;
        }
        img{
            margin-top:-20px;
                margin-left: 775px;
                width: 300px;
                height: 250px;
        }
        #Panel1
        {
            background-color:aliceblue;
            font-size:30px;
            position: relative;
            left: 40px;
            top: 0px;
            width: 500px;
            height: 550px;
            text-align:start;
        }
        #nameBox{
            width: 600px;
            height: 25px;
            position: relative;
            top: -20px;
            font-size:20px;
        }
         #TextBox2{
            width: 600px;
            height: 25px;
            position: relative;
            top: -20px;
            font-size:20px;

        }
         #TextBox3{
            width: 600px;
            height: 25px;
            position: relative;
            top: -20px;
            font-size:30px;
        }        
         #TextBox4{
            width: 600px;
            height: 25px;
            position: relative;
            top: -20px;
            font-size:30px;
         }
         .ABC
        {
            margin-top: 5px;
            font-size: 20px;
            position: relative;
            top: -15px;
            left: 0px;
        }
         p{
             font-size:25px;
             margin-top:-10px;
         }
         #Button1{
             width:610px;
             height:40px;
             background-color:orange;
             font-size:25px;
         }
         .tandc{
             font-size:20px;
             font-family:"sans-serif";
             position:relative;
             top:18px;
         }
         .signin{
             font-size:26px;
             position:relative;
             top:50px;
             left:100px;
         }
         
        .auto-style1 {
            left: 0px;
            top: -20px;
        }
         
    </style>
</head>
<body>
    <form id="form1" runat="server">
        <div>
            <a href="https://localhost:44361/MasterForm.aspx">
              <img alt="problem in image" src="data:image/png;base64," class="auto-style9" />
            </a>
        </div>

        <div class="signup">
            <asp:Panel ID="Panel1" runat="server">
                <h2>Sign-Up</h2>
                <p style ="margin-top:-10px ">Your name</p>
                <asp:TextBox ID="nameBox" onclick="clickfun()" runat="server" CssClass="auto-style1"></asp:TextBox>
                <p style ="margin-top:5px ">Email/Phone number</p>
                <asp:TextBox ID="TextBox2" runat="server"></asp:TextBox>
                <p style ="margin-top:5px ">Password</p>
                <asp:TextBox ID="TextBox3" runat="server" TextMode="Password"></asp:TextBox>
                 <p class ="ABC">Password must be atleast 6 characters</p>
                <p style ="margin-top:5px ">Re-enter Password</p>
                <asp:TextBox ID="TextBox4" runat="server" TextMode="Password"></asp:TextBox>
                <asp:Button ID="Button1" runat="server" Text="Submit" OnClick="Button1_Click" />

                <p class="tandc">By continuing, you agree to Amazon's<a href="https://localhost:44361/termsAndCondi.aspx"> Conditions of Use</a> and <a href="https://localhost:44361/termsAndCondi.aspx"> Privacy Notice</a></p>
                <p class ="signin">Already have an account? <a href="https://localhost:44361/signinPage.aspx"> Sign in -></a></p>
            </asp:Panel>
            
        </div>
    </form>
</body>
</html>
