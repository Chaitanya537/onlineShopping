l<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MasterForm.aspx.cs" Inherits="onlineShopping.MasterForm" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Amazon online shopping site for gadgets, clothing, design, house hold applications etc</title>
    <style type="text/css">
        .auto-style2 {
            width: 1438px;
            height: 975px;
        }
        .auto-style6 {
            position:relative;
            top:-70px;
            background-color:ghostwhite;
            width: 859px;
            height: 950px;
            margin-left: 500px;
            margin-top:70px;
            border-color:black;  
            border-style:groove;
        }      
        .auto-style7 {
            margin-left: 228px;
            margin-top: 18px;
        }
        img {
            margin-left:75px;
            width: 160px;
            height: 180px;
        }
        .auto-style9 {
            width: 395px;
            height: 243px;
            margin-left: 11px;
            margin-top: 0px;
        }
        h3{
            position:relative;
            left:10px;
            font-size:35px;
            font-family:sans-serif;
            
        }
        .auto-style10 {
            left: 0px;
            top: -23px;
            height: 60px;
        }
        ul {
            position:relative;
            list-style-type: none;
            left:-110px;
            font-size:22px;
            padding-left:5em;
            margin: 0;
            line-height:1.5em;
            overflow: hidden;
            font-family:sans-serif;
        }
        .auto-style11 {
            left: -181px;
            top: -45px;
            height: 132px;
            width: 524px;
        }
        .auto-style12 {
            margin-left: 0px;
            margin-top: 0px;
            left: -90px;
            top: -18px;
            width: 623px;
            height: 46px;
        }
        #Button1
        {
            position:relative;
            left:-90px;
            font-size:20px;
            background-color:orange;
            font-weight:500;
            border-block:groove;
            border-block-color:darkorange;
            cursor:pointer;
        }
        #Button2
        {
            position:relative;
            left:-90px;
            font-size:20px;
            margin-top:-5px;
            font-weight:500;
            border-block:groove;
            border-block-color:silver;
            cursor:pointer;
        }
        #Button3
        {
            position:relative;
            left:-90px;
            font-size:20px;
            margin-top:25px;
            font-weight:500;
            border-block:groove;
            border-block-color:silver;
            cursor:pointer;
        }
        .auto-style13 {
            margin-left: 0px;
            margin-top: 0px;
            left: -90px;
            top: -34px;
            width: 623px;
            height: 54px;
        }
        p,a
        {
            font-size:25px;
            position:relative;
            left:-40px;
        }
        .auto-style14 {
            height: 88px;
            margin-left: 0px;
            left: -110px;
            top: 57px;
            width: 755px;

        }
    </style>
</head>
<body>
    <form id="form1" runat="server" class="auto-style2">       
        <div class="auto-style6">
            <asp:Panel ID="Panel1" runat="server" CssClass="auto-style7" Height="251px" Width="479px">
                <img alt="problem in image" src="data:image/png;base64," class="auto-style9" />
                <h3 class="auto-style10">Sign in to your account</h3>
                <ul class="auto-style11">
                    <li>view your wish list</li>
                    <li>Find & reorder past purchases</li>
                    <li>Track your purchases</li>
                </ul>
                <asp:Button ID="Button1" runat="server" Text="Already a customer? Sign in" CssClass="auto-style13" OnClick="Button1_Click" />
                <asp:Button ID="Button2" runat="server" Text="New to Amazon.in? Create an account" CssClass="auto-style12" OnClick="Button2_Click" />
                <asp:Button ID="Button3" runat="server" Text="Skip Sign in" CssClass="auto-style13" OnClick="Button3_Click" />
                <p class="auto-style14">By continuing, you agree to Amazons &#39&nbsp&nbsp;&nbsp;&nbsp;&nbsp; <a href="https://localhost:44361/termsAndCondi.aspx">Conditions of Use </a> and &nbsp&nbsp&nbsp&nbsp; <a href="https://localhost:44361/termsAndCondi.aspx">Privacy Notice</a> </p>
                <p> &nbsp;</p>
            </asp:Panel>         
        </div>
    </form>
</body>
</html>
