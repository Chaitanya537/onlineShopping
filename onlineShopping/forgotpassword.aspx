<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="forgotpassword.aspx.cs" Inherits="onlineShopping.forgotpassword" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">

    <title>
    </title>
    <style>
        img{
            margin-top:-20px;
                margin-left: 775px;
                width: 300px;
                height: 250px;
        }
        .forgotPassword{
            background-color:whitesmoke;
            position: relative;
            margin-left: 575px;
            margin-top: -60px;
            border:solid;
            border-style:groove;
            width: 700px;
            height: 730px; 
            text-align:start;
        }
        #Panel1{
            font-size:30px;
            position: relative;
            left: 40px;
            top: 10px;
            width: 600px;
            height: 600px;
            text-align:start;
        }
        #mailBox{
            width: 600px;
            height: 40px;
            position: relative;
            top: -20px;
            font-size:20px;        
        }
        #passdBox{
            width: 600px;
            height: 40px;
            position: relative;
            top: -20px;
            font-size:30px;
        }
        #Button1{
            width: 610px;
            height: 60px;
            position: relative;
            margin-top:10px;
            font-weight:500;
            font-size: 27px;
            background-color:orange;
            cursor:pointer;
        }
        #otp{
            margin-top:100px;

        }
        #TextBox3{
            position: relative;
            bottom: 70px;
            left: 175px;
            width: 100px;
            height: 40px;
            font-size:20px;
        }
        #mssg{
            font-size:25px;
            position:relative;
            top:20px;    
        }
        #Button2{
            width: 100px;
            height: 50px;
            position: relative;
            top: -117px;
            left:315px;
        }
        #demo{
            text-align: right;
            position: relative;
            top: 80px;
            display:none;
        }
        #TextBox4{
                width:40px;
                position: relative;
                left: 500px;
                height: 20px;
                top: 100px;
        }
        .auto-style1 {
            left: 1px;
            top: -20px;
        }
        #lbl1{
                position: relative;
                top: 100px;
        }
        #TextBox1{
                width: 40px;
                position: relative;
                top: 100px;
        }
    </style>
    <script>
        let a = 0;
        function loadpg() {
          
            document.getElementById("mssg").style.display = 'none';
            document.getElementById("otp").style.display = 'none';
            document.getElementById("TextBox3").style.display = 'none';
            document.getElementById("Button2").style.display = 'none';
          
        }
        function randNum() {
            document.getElementById("mssg").style.display = 'block';
            document.getElementById("otp").style.display = 'block';
            document.getElementById("TextBox3").style.display = 'block';
            document.getElementById("Button2").style.display = 'block';
        }
    </script>
</head>
<body onload ="loadpg()" >
    
    <form id="form1" runat="server">
        <div>
            <a href="https://localhost:44361/MasterForm.aspx">
              <img alt="problem in image" src="data:image/png;base64," class="auto-style9" />
            </a>
        </div>
        
        <div class="forgotPassword">
            <asp:Panel ID="Panel1" runat="server">
                <h2>Forgot Password</h2>
                <p style ="margin-top:-10px ">Enter User Name</p>
                <asp:TextBox ID="mailBox" onclick = "clickevent()" runat="server"></asp:TextBox>
                <p style ="margin-top:5px ">Enter Email (phone for mobile accounts)</p>
                <asp:TextBox ID="passdBox" runat="server" CssClass="auto-style1"></asp:TextBox>
                
                <asp:Button ID="Button1" runat="server" Text="Submit" OnClick="Button1_Click" />
                <input id="Button3" onclick ="randNum()" type="button" value="click here" />
                <p id ="mssg">we have sent an otp to your registered mobile number / Email account</p>
                <p id="otp">Enter OTP : </p>
                <asp:TextBox ID="TextBox3" runat="server" TextMode="Number"></asp:TextBox>
                <asp:Button ID="Button2" runat="server" Text="Submit" CssClass="auto-style1" OnClick="Button2_Click1" />

            </asp:Panel>
            <label id ="lbl1">Account Id :</label>
            <asp:TextBox ID="TextBox1" runat="server"></asp:TextBox>
            <asp:TextBox ID="TextBox4" runat="server"></asp:TextBox>
        </div>
    </form>
</body>
</html>
